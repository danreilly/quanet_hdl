...
To move data, qnicll uses AD's libiio library.  In the PL (programmable logic) of the FPGA, "qregs" means "quanet regs", and it's the small set of registers we added in order to implement the LFSR-based probe generation, the syncronized transmit and recieve, and the driver for the fast optical switch for noise measurements.  (library/quanet_regs).  Libiio knows nothing about these, so we wrote a driver "qsrvr" that mmaps the registers so they can be used.  qnicll connects to qsrvr to access these registers.  Eventually we can access these through PCI, and we can get rid of qsrvr.  Eventually we can get also rid of the USB connection to the arty S750 board, and use the rs422 link shown in yellow (this is not currently used).
