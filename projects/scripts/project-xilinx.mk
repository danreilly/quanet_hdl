...
all: h_vhdl_include.h external_dependencies $(PROJECT_NAME).sdk/system_top.xsa
...
$(PROJECT_NAME).sdk/system_top.xsa: $(M_DEPS)
	...
	rm -f $(PROJECT_NAME)_vivado.log
	$(call build, $(VIVADO) system_project.tcl, $(PROJECT_NAME)_vivado.log, \
		$(HL)$(PROJECT_NAME)$(NC) project)


$(PROJECT_NAME).sdk/system_top.xsa_PRIOR: $(M_DEPS)
	if [ $(MODE) = incr ]; then \
		if [ -f */impl_1/system_top_routed.dcp ]; then \
			echo Found previous run result at `ls */impl_1/system_top_routed.dcp`; \
			cp -u */impl_1/system_top_routed.dcp ./reference.dcp ; \
	...
	echo DAN: building .xsa
	$(call skip_if_missing, \
		Project, \
		$(PROJECT_NAME), \
		true, \
	rm -rf $(CLEAN_TARGET) ; \
	$(call build, $(VIVADO) system_project.tcl, $(PROJECT_NAME)_vivado.log, 		$(HL)$(PROJECT_NAME)$(NC) project)


# This is used to make each of the IPs.
